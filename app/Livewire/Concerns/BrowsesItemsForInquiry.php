...
/**
 * Sales-order style F2 item browse for inquiry pages (single-item pick).
 */
trait BrowsesItemsForInquiry
{
    ...
    {
        $this->browseSearch = trim((string) ($this->itemCode ?? ''));
        $this->lookupError = '';
        $this->lineWarning = '';
    ...
    public function insertBrowseSelected(): void
    {
        ...
        if ($id === null) {
            $this->lineWarning = count($this->browseCheckedIds) > 1
                ? 'Multiple items checked — uncheck to a single item, or double-click a row.'
     ...
     */
    protected function inquiryBrowseViewData(): array
    {
        $companyId = (int) auth()->user()->company_id;
        ...
        return [
            ...
            'oversellingOn' => true,
            'browseSearchPlaceholder' => 'Type or scan code — Enter to select',
        ];
    }
}
