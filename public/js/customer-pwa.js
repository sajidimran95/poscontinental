...
(function () {
  'use strict';
  var cfg = window.__cust_pwa__ || {};
  var KEY = 'cust_pwa_installed';
  var DISMISS_KEY = 'cust_pwa_dismissed_at';
  ...
  function setBar(show) {
    var bar = document.getElementById('cust_pwa_install_bar');
    if (!bar) return;
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register(cfg.swUrl || '/customer/pwa/sw.js', { scope: '/customer/' }).catch(function () {});
  }
  ...
  document.addEventListener('click', function (e) {
    if (e.target.closest('#cust_pwa_dismiss_btn')) {
      markDismiss();
      setBar(false);
      return;
    }
    var btn = e.target.closest('#cust_pwa_install_btn');
    if (!btn) return;
    e.preventDefault();
    if (deferred) {
      deferred.prompt();
